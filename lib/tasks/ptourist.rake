namespace :ptourist do
  ...
  desc "reset things, images, and links, types"
...
"}
    ...
    {Museum: ["B&O Railroad Museum"],
     Aquarium: ["Baltimore Water Taxi", "National Aquarium"],
     Rental: ["Rent-A-Tour"],
     Hotel: ["Holiday Inn Timonium", "Hyatt Place Baltimore"]
   }.each do |type, things|
     type = Type.create(name: type);
     things.each {|thing| type.things << Thing.where(name: thing)}
     type.save!
   end
  end
